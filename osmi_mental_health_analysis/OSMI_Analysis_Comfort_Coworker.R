...
#investigate workplace conditions vs comfort discussing (coworker)#
osmi_comfort <- osmi_us %>% 
  select(5:13, 15:16, 18) %>% 
  drop_na(comfort_supervisor, comfort_coworker)

#plot comfort discusisng mh coworker vs mh coverage#
...
#plot com_cow vs mh options aware#
...
#plot com_cow vs employer discuss mh#
...
#plot com_cow vs employer offer mh#
...
#plot com_cow vs anon protection#
...
osmi_lea_com_cow <- osmi_comfort %>% 
  ...
  geom_bar() +
  labs(x = "Difficulty Taking MH Leave", y = "", fill = "", title = "Comfort Discussing Mental Health with Coworker")
osmi_lea_com_cow

#plot all graphs together#
grid.arrange(osmi_cov_com_cow, osmi_opt_com_cow, osmi_empdis_com_cow, 
             osmi_resoff_com_cow, osmi_anon_com_cow, osmi_lea_com_cow, 
             ncol = 2)